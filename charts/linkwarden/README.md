# FORK of FMJ Studios - Linkwarden Helm Chart

```
helm repo add taliamarine https://taliamarine.github.io/helm-charts/
helm install linkwarden taliamarine/linkwarden --version X.Y.Z
```

The purpose is to fix SSO with Authentik using existing secret.
...
| `linkwarden.database.existingSecret`            | The name of an existing secret containing the full PostgreSQL connection URL                                                                           | `""`         |
| `linkwarden.database.urlKey`                    | The key within `existingSecret` that holds the complete PostgreSQL connection URL (matches operators like CloudNativePG)                               | `uri`        |
